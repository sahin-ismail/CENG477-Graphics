...
namespace fst
{
    ...
    math::Vector3f Integrator::renderPixel(const Ray& ray, int depth) const
    {
        ...
        for (auto& light : m_scene.point_lights)
        {
            ...
            if(0 > hit_record.id_of_text ){
                if (!m_scene.intersectShadowRay(shadow_ray, light_pos_distance))
                {
                    color = color + light.computeRadiance(light_pos_distance) * material.computeBrdf(to_light, -ray.get_direction(), hit_record,this->m_scene.textures,this->m_scene.tex_coord_data);

                }
            }
            if(0 < hit_record.id_of_text ){
                const Texture &text_data = this->m_scene.textures[hit_record.id_of_text-1];
                if(text_data.m_decalMode == "replace_all") {
                    color = material.computeBrdf(to_light, -ray.get_direction(), hit_record,this->m_scene.textures,this->m_scene.tex_coord_data);
                    break;
                }
                if(text_data.m_decalMode != "replace_all") {
                    if(!m_scene.intersectShadowRay(shadow_ray, light_pos_distance))
